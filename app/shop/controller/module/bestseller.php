...
class ControllerModuleBestSeller extends Controller {
	protected function index($widget=null) {
    	...
    	$limit = ((int)$settings['limit']) ? (int)$settings['limit'] : $this->config->get('latest_limit');
		$this->prefetch($limit);
        
		$this->id = 'bestseller';

		if ($widget['position'] == 'main' || $widget['position'] == 'featuredContent') {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/module/bestseller_home.tpl')) {
				$this->template = $this->config->get('config_template') . '/module/bestseller_home.tpl';
			} else {
				$this->template = 'default/module/bestseller_home.tpl';
			}
		} else {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/module/bestseller.tpl')) {
				$this->template = $this->config->get('config_template') . '/module/bestseller.tpl';
			} else {
				$this->template = 'default/module/bestseller.tpl';
			}
		}
    ...
    protected function prefetch($limit) {
		...
		$results = $this->modelProduct->getBestSellerProducts($this->config->get('bestseller_limit'));
    ...
    public function carousel() {
        ...
        $json['results'] = $this->modelProduct->getBestSellerProducts(isset($_GET['limit']) ? $_GET['limit'] : 40);
        ...
        foreach ($json['results'] as $k => $v) {
            ...
            $json['results'][$k]['href'] = Url::createUrl('store/product',array('product_id'=>$json['results'][$k]['product_id']));

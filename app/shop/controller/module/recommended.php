...
class ControllerModuleRecommended extends Controller {
	protected function index($widget=null) {
    	...
    	$limit = ((int)$settings['limit']) ? (int)$settings['limit'] : $this->config->get('recommended_limit');
		$this->prefetch($limit);
        
		$this->id = 'recommended';

		if ($widget['position'] == 'main' || $widget['position'] == 'featuredContent') {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/module/recommended_home.tpl')) {
				$this->template = $this->config->get('config_template') . '/module/recommended_home.tpl';
			} else {
				$this->template = 'default/module/recommended_home.tpl';
			}
		} else {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/module/recommended.tpl')) {
				$this->template = $this->config->get('config_template') . '/module/recommended.tpl';
			} else {
				$this->template = 'default/module/recommended.tpl';
			}
		}
    ...
    public function carousel() {
        ...
        $json['results'] = $this->modelProduct->getRecommendedProducts(isset($_GET['limit']) ? $_GET['limit'] : 40);

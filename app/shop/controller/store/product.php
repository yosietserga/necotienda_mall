...
class ControllerStoreProduct extends Controller {
    ...
    public function index() {
        ...
        if (($product_info['status'] == 1 && $customer_info['banned'] == 0 && $product_info['paid'] == 1) || ($this->customer->getId() == $product_info['owner_id'])) {
            ...
            if ($cached && !$this->user->isLogged() && !$this->request->hasQuery('np')) {
                ...
                if ($this->request->hasQuery('p_email') || $this->request->hasQuery('p_customer_id')) {
                    if ($this->request->hasQuery('p_email')) {
                        $p_customer = $this->modelCustomer->getCustomerByEmail($this->request->getQuery('p_email'));
                    } else {
                        $p_customer = $this->modelCustomer->getCustomer($this->request->getQuery('p_customer_id'));
                    }
                    $this->modelProduct->productPromoted($product_id, $p_customer['customer_id']);
                 }
                ...
                if (isset($this->request->get['q'])) {
                    ...
                    $this->document->breadcrumbs[] = array(
                        'href' => $Url::createUrl('store/search', array('q'=>$this->request->get['q'])) . $url,
                ...
                $this->document->breadcrumbs[] = array(
                    'href' => $Url::createUrl('store/product', array('product_id' => $product_id)) . $url,
    ...
    protected function error404() {
        ...
        $this->document->breadcrumbs[] = array(
            'href' => $Url::createUrl('store/product', array('product_id' => $this->product_id)) . $url,
    ...
    public function description() {
        $this->load->auto('store/product');
        $product_id = ($this->request->hasQuery('product_id')) ? (int) $this->request->getQuery('product_id') : $product_id = 0;
        $product_info = $this->modelProduct->getProduct($product_id);
        if ($product_info['status'] == 1 || ($this->customer->getId() == $product_info['owner_id'])) {
            $this->response->setOutput(html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8'), $this->config->get('config_compression'));
        }
    }
    ...
    public function likeReview() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->customer->islogged() && $review_id && $product_id) {
            ...
            $json['success'] = 1;
            if (!$this->modelCustomer->getProperty($this->customer->getId(), 'rewards', 'like_a_review') && $this->customer->isLogged()) {
                $this->modelCustomer->setProperty($this->customer->getId(), 'rewards', 'like_a_review', 1);
                $this->modelCustomer->addNecoexp($this->customer->getId(), 5);
            }
        }
    ...
    public function write() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if (!$this->modelCustomer->getProperty($this->customer->getId(), 'rewards', 'write_a_review') && $this->customer->isLogged()) {
                $this->modelCustomer->setProperty($this->customer->getId(), 'rewards', 'write_a_review', 1);
                $this->modelCustomer->addNecoexp($this->customer->getId(), 7);
            }
    ...
    public function reply() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateReply()) {
            ...
            if (!$this->modelCustomer->getProperty($this->customer->getId(), 'rewards', 'reply_a_review') && $this->customer->isLogged()) {
                $this->modelCustomer->setProperty($this->customer->getId(), 'rewards', 'reply_a_review', 1);
                $this->modelCustomer->addNecoexp($this->customer->getId(), 5);
            }
    ...
    public function relatedJson() {
        ...
        foreach ($json['results'] as $k => $v) {
            if (!file_exists(DIR_IMAGE . $v['pimage']))
                $json['results'][$k]['image'] = HTTP_IMAGE . "no_image.jpg";
            $json['results'][$k]['thumb'] = NTImage::resizeAndSave($v['pimage'], $width, $height);
    ...
    public function productcalled() {
        ...
        if ($product_id) {
            ...
            if (!$this->modelCustomer->getProperty($this->customer->getId(), 'rewards', 'product_called') && $this->customer->isLogged()) {
                $this->modelCustomer->setProperty($this->customer->getId(), 'rewards', 'product_called', 1);
                $this->modelCustomer->addNecoexp($this->customer->getId(), 5);
            }

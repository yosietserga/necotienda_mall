...
class ControllerApiGoogle extends Controller {
    ...
    public function invitefriends() {
        if ($this->google->getAccessToken()) {
            ...
            if ($this->config->get('marketing_email_invite_friends')) {
                ...
                if (!$this->modelCustomer->getProperty($this->customer->getId(), 'rewards', 'website_promoted_by_google')) {
                    $this->modelCustomer->setProperty($this->customer->getId(), 'rewards', 'website_promoted_by_google', 1);
    ...
    public function promote() {
        if ($this->google->getAccessToken()) {
            ...
            $xml = simplexml_load_string($this->google->getAuth()->authenticatedRequest($req)->getResponseBody());
            $response = json_encode($xml);
            ...
            if ($this->config->get('marketing_email_promote_product')) {
                ...
                if ($product_id) {
                    ...
                    if ($product) {
                        ...
                        $product['url'] = $Url::createUrl('store/product', array(
                            'product_id' => $product['product_id'],
                            'cc' => $this->currency->getCode(),
                            'hl' => $this->config->get('config_language'),
                            'p_email' => $userInfo->email,
                            'p_ref' => 'gmail',
                            'p_contact_id' => $contact_id,
                            'p_customer_id' => (int)$customer->row['customer_id']
                        ));
                ...
                if (!$this->modelCustomer->getProperty($this->customer->getId(), 'rewards', 'product_promoted_by_google')) {
                    $this->modelCustomer->setProperty($this->customer->getId(), 'rewards', 'product_promoted_by_google', 1);
    ...
    public function login() {
        ...
        if ($this->google->getAccessToken()) {
            ...
            $company = ($userInfo->name) ? $userInfo->name : $userInfo->given_name . $userInfo->family_name;
            $data = array(

...
class ControllerAccountHeader extends Controller {
	protected function index() {
        ...
        // style files
        $csspath = defined("CDN_CSS") ? CDN_CSS : HTTP_PROFILE_CSS;
        ...
        if ($this->data['css']) {
            ...
            $this->data['css'] = str_replace("../images/",str_replace('%theme%',$this->config->get('config_template'),HTTP_PROFILE_IMAGE),$this->data['css']);
            $this->data['css'] = str_replace("../fonts/",str_replace('%theme%',$this->config->get('config_template'),HTTP_PROFILE_FONTS),$this->data['css']);
        }
		...
		$this->id = 'header';
        $this->template = 'default/account/header.tpl';
    	$this->render();
	}
}
...
class ControllerSaleCreate extends Controller { 
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate()) {
            ...
            if (is_array($this->request->post['Images'])) {
                ...
                $this->request->post['price'] = str_replace(',','.',str_replace('.','',$this->request->getPost('price')));
                    echo $this->request->post['price'].'<br>';
                $plan = $this->modelPlan->getPlan($this->request->getPost('plan_id'));echo __LINE__.'<br>';
                $product_id = $this->modelProduct->add($this->request->post);echo __LINE__.'<br>';
                echo __LINE__.'<br>';
        		$data = array();
        		echo __LINE__.'<br>';
    ...
    protected function validate() {
        ...
        if (empty($this->request->post['model'])) {
            $this->error['model'] = $this->language->get('error_model_empty');
        }
        
        if (empty($this->request->post['meta_description'])) {
            $this->error['meta_description'] = $this->language->get('error_meta_description_empty');
        }
	...
	protected function upload() {
		...
		foreach ($files as $key => $file) {
            ...
            if ($file['error'] == UPLOAD_ERR_INI_SIZE) return $this->language->get('UPLOAD_ERR_INI_SIZE');
            if ($file['error'] == UPLOAD_ERR_FORM_SIZE) return $this->language->get('UPLOAD_ERR_FORM_SIZE');
            if ($file['error'] == UPLOAD_ERR_PARTIAL) return $this->language->get('UPLOAD_ERR_PARTIAL');
            if ($file['error'] == UPLOAD_ERR_NO_FILE) return $this->language->get('UPLOAD_ERR_NO_FILE');
            if ($file['error'] == UPLOAD_ERR_NO_TMP_DIR) return $this->language->get('UPLOAD_ERR_NO_TMP_DIR');
            if ($file['error'] == UPLOAD_ERR_CANT_WRITE) return $this->language->get('UPLOAD_ERR_CANT_WRITE');
            if ($file['error'] == UPLOAD_ERR_EXTENSION) return $this->language->get('UPLOAD_ERR_EXTENSION');

...
class ControllerProfileSales extends Controller {
    ...
    public function rateBox() {
        ...
        if ($this->request->hasQuery('profile_id')) {
            ...
            if ($model['customer_id']) {
                $data['ratings'] = $this->modelCustomer->getSellerRatings($model['customer_id']);
                $data['qty_orders'] = $this->modelOrder->getTotalSalesWaitingByCustomerId($model['customer_id']);
                $data['qty_sales'] = $this->modelOrder->getTotalSalesCompleteByCustomerId($model['customer_id']);
                $data['qty_returns'] = $this->modelOrder->getTotalSalesReturnsByCustomerId($model['customer_id']);
                $data['products'] = $this->modelProduct->getTotalProductsByCustomerId($model['customer_id'], array('status'=>1));
            }
        }

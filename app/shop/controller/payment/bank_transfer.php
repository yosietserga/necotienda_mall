...
class ControllerPaymentBankTransfer extends Controller {
	protected function index() {
        ...
        $result = $this->modelNewsletter->getById($this->config->get('bank_transfer_newsletter_id'));
        ...
        $results = $this->db->query("SELECT * FROM ". DB_PREFIX ."bank b");
        $this->data['banks'] = $results->rows;
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/payment/bank_transfer.tpl')) {
			...
			$this->template = 'default/payment/bank_transfer.tpl';
		}	
	...
	public function confirm() {
        ...
        if ($this->session->has('order_id')) {
            ...
            $this->data['order_id'] = $this->request->getQuery('order_id');
        } elseif ($this->request->hasPost('bank_transfer_order_id')) {
            $this->data['order_id'] = $this->request->getPost('bank_transfer_order_id');
        } else {
            $this->data['order_id'] = 0;
        }
        ...
        if ($order_info && $this->customer->isLogged()) {
            ...
            $amount = explode(",",$this->request->getPost('bank_transfer_amount'));
            $payment = round(str_replace(".","",$amount[0]) .".". $amount[1],2);
            
            $data['order_id']       = $order_info['order_id'];
            $data['bank_account_id']= $this->request->getPost('bank_transfer_bank_account_id');
            $data['bank_from']      = $this->request->getPost('bank_transfer_bank_id');
            $data['transac_number'] = $this->request->getPost('bank_transfer_transact');
            $data['transac_date']   = date('Y-m-d',strtotime($this->request->getPost('bank_transfer_date_added')));
            $data['payment_method'] = $this->language->get('text_title');
            $data['amount']         = $payment;
            $data['comment']        = $this->request->getPost('bank_transfer_comment');
            $data['order_status_id']= $this->config->get('bank_transfer_order_status_id');
            ...
            $this->modelOrder->updateStatus($order_info['order_id'],$this->config->get('bank_transfer_order_status_id'));
            ...
            $mailer = new Mailer;
            if ($this->config->get('bank_transfer_newsletter_id')) {
                $this->load->model("marketing/newsletter");

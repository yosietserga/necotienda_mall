...
<form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
    <table id="list">
        ...
        <?php if ($plans) {  foreach ($plans as $plan) { ?>
            <tr id="tr_<?php echo $plan['plan_id']; ?>">
                <td><input title="Seleccionar para una acci&oacute;n" type="checkbox" name="selected[]" value="<?php echo $plan['plan_id']; ?>" <?php if ($plan['selected']) { ?>checked="checked"<?php } ?>/></td>
                ...
                <td>
                <?php foreach ($plan['action'] as $action) {  if ($action['action'] == "activate") { 
                        $jsfunction = "activate(". $plan['plan_id'] .")";
                        $href = "";
                    } elseif ($action['action'] == "delete") {
                        $jsfunction = "eliminar(". $plan['plan_id'] .")";
                        $href = "";
                ...
                ?>
                <a title="<?php echo $action['text']; ?>" <?php echo $href; ?> onclick="<?php echo $jsfunction; ?>"><img id="img_<?php echo $plan['plan_id']; ?>" src="image/<?php echo $action['img']; ?>" alt="<?php echo $action['text']; ?>" /></a>
                <?php } ?>
                </td>
            </tr>
            <?php }  } else { ?>

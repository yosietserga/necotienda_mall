  ...
  <div class="content">  
    ...
    <thead>
      ...
      <tr class="filter">
        ...
        <?php if ($manufacturer['percent'] < 1) { ?>            
           <tr style="background: #FFF0F0;">
          <?php } else { ?>            
          <tr style="font-weight:bold;background: #F4FFF0">
          <?php } ?>     
          <td class="left"><a href="<?php echo HTTP_HOME."index.php?r=sale/customer/update&token=".$_GET['token']."&customer_id=".$manufacturer['customer_id']; ?>"><?php echo $manufacturer['email']; ?></a></td>
    ...
    <thead>
      ...
      <tbody>
        <?php if ($manufacturers['detail']) {  ?>
        <?php foreach ($manufacturers['detail'] as $detail) { ?>
        <tr>    
...
<script type="text/javascript">
function filter() {
	url = 'index.php?r=report/mviewedbycustomer&token=<?php echo $_GET["token"]; ?>&manufacturer_id=<?php echo $_GET["manufacturer_id"]; ?>';

<?php
/**
 * ControllerModuleInviteFriendsWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleInviteFriendsWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleInviteFriendsWidget::index()
	 * 
	 * @return
	 */
	public function index() {
		...
		$this->load->language('module/invitefriends');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/invitefriends';
                $settings->autoload = 1;
                $data['settings']   = $settings;
                $widget->save($data);
                $this->setvar('name');
    		...
    		$this->template = 'module/invitefriends/widget.tpl';

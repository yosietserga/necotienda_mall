...
class ControllerTotalTotal extends Controller { 
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			$this->modelSetting->update('total', $this->request->post);
		
			$this->session->set('success',$this->language->get('text_success'));
			
            if ($_POST['to'] == "saveAndKeep") {
                $this->redirect(Url::createAdminUrl('total/total')); 
            } else {
		...
		$this->data['tab_general'] = $this->language->get('tab_general');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => Url::createAdminUrl('total/total'),
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = Url::createAdminUrl('total/total');

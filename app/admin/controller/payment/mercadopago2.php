...
class ControllerPaymentMercadopago2 extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			$this->model_setting_setting->update('mercadopago2', $this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect(HTTP_HOME . 'index.php?route=extension/payment&token=' . $this->session->data['token']);
		}
		...
		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTP_HOME . 'index.php?route=common/home&token=' . $this->session->data['token'],
       		'text'      => $this->language->get('text_home'),
      		'separator' => FALSE
   		);

   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTP_HOME . 'index.php?route=extension/payment&token=' . $this->session->data['token'],
       		'text'      => $this->language->get('text_payment'),
      		'separator' => ' :: '
   		);

   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTP_HOME . 'index.php?route=payment/mercadopago2&token=' . $this->session->data['token'],
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);

		$this->data['action'] = HTTP_HOME . 'index.php?route=payment/mercadopago2&token=' . $this->session->data['token'];

		$this->data['cancel'] = HTTP_HOME . 'index.php?route=extension/payment&token=' . $this->session->data['token'];
		...
		$this->data['countries'] = $this->getAll();
		...
		$this->data['order_statuses'] = $this->model_localisation_order_status->getAll();

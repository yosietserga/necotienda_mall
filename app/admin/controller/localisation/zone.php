...
class ControllerLocalisationZone extends Controller {
	...
	private function getList() {
		...
		$results = $this->model_localisation_zone->getAll($data);
	...
	private function getForm() {
		...
		if (isset($this->request->get['zone_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$zone_info = $this->model_localisation_zone->getById($this->request->get['zone_id']);
		}
		...
		$this->data['countries'] = $this->model_localisation_country->getAll();
	...
	private function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $zone_id) {
			...
			$zone_to_geo_zone_total = $this->model_localisation_geo_zone->getAllTotalByZoneId($zone_id);

...
class ControllerLocalisationCountry extends Controller {
	...
	private function getList() {
		...
		$country_total = $this->model_localisation_country->getAllTotal();
		
		$results = $this->model_localisation_country->getAll($data);
	...
	private function getForm() {
		...
		if (isset($this->request->get['country_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$country_info = $this->model_localisation_country->getById($this->request->get['country_id']);
		}
	...
	private function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $country_id) {
			...
			$zone_to_geo_zone_total = $this->model_localisation_geo_zone->getAllTotalByCountryId($country_id);

...
class ControllerModuleLatest extends Controller {
	protected function index($widget=null) {
		...
		if ($widget['position'] == 'main') {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/module/latest_home.tpl')) {
				$this->template = $this->config->get('config_template') . '/module/latest_home.tpl';
			} else {
				$this->template = 'default/module/latest_home.tpl';
			}
		} else {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/module/latest.tpl')) {
				$this->template = $this->config->get('config_template') . '/module/latest.tpl';
			} else {
				$this->template = 'default/module/latest.tpl';
			}
		}
		$this->render();
	}
    ...
    public function home() {
        $this->prefetch($this->config->get('latest_limit'));
    ...
    public function carousel() {
        ...
        $json['results'] = $this->modelProduct->getLatestProducts(isset($_GET['limit']) ? $_GET['limit'] : 40);

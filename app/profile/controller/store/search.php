...
class ControllerStoreSearch extends Controller { 	
	public function index($q) {
        ...
        if ($cached && !$this->user->isLogged()) {
            ...
            $this->data['urlCriterias']['forCategories'] = 
            $this->data['urlCriterias']['forZones'] = 
            $this->data['urlCriterias']['forSellers'] = 
            $this->data['urlCriterias']['forManufacturers'] = 
            $this->data['urlCriterias']['forStores'] = 
            $this->data['urlCriterias']['forPrices'] = 
            $this->data['urlCriterias']['forShipping'] = 
            $this->data['urlCriterias']['forPayments'] = 
            $this->data['urlCriterias']['forStatus'] = 
            $this->data['urlCriterias']['forStockStatus'] = 
            $this->data['urlCriterias']['forDates'] = $queries[1];
            
        	$this->document->title = str_replace('-',' ',$keyword);
            ...
            if ($queries[1] != $queries[2]) {
                ...
                if (str_word_count($queries[2],0) > 1) $queries[4] = str_replace(' ','',$queries[2]);
            } else {
                unset($queries[2]);
            }
            
            if (str_word_count($queries[1],0) > 1) $a1 = explode(' ',$queries[1]);
            if ($queries[2]) $a2 = explode(' ',$queries[2]);
            
            if ($a1) $queries = array_merge($queries,$a1);
            if ($a2) $queries = array_merge($queries,$a2);
    		...
    		if ($total) {
    		    ...
    		    if (!$criteria['categories']) $this->data['filterCategories'] = $this->modelSearch->getCategoriesByProduct($criteria);
    		    if (!$criteria['manufacturer']) $this->data['filterManufacturers'] = $this->modelSearch->getManufacturersByProduct($criteria);
    		    if (!$criteria['seller']) $this->data['filterSellers'] = $this->modelSearch->getSellersByProduct($criteria);
    		    if (!$criteria['zone']) $this->data['filterZones'] = $this->modelSearch->getZonesByProduct($criteria);
    		    if (!$criteria['stores']) $this->data['filterStores'] = $this->modelSearch->getStoresByProduct($criteria);

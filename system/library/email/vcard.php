...
final class vCard
  ...
  */
  function vCard($config,$downloaddir = DIR_DOWNLOAD, $lang = 'es') {
    $this->config = (object)$config;
    $this->download_dir = $downloaddir;
    $this->card_filename = 'vcard.vcf';
    $this->rev = (string) date('dmY\THi00\Z',time());
    $this->setLanguage($lang);
    if ($this->checkDownloadDir() == false) die('error creating download directory'); // end if
  } // end function
  
  function getOwnerVCard() {
    $this->setFirstName($this->config->get('config_owner'));
    $this->setLastName($this->config->get('config_owner'));
    $this->setNickname($this->config->get('config_owner'));
    $this->setCompany($this->config->get('config_name'));
    $this->setOrganisation($this->config->get('config_name'));
    $this->setDepartment('Ventas');
    $this->setJobTitle('Gerencia de Ventas');
    $this->setTelephoneWork1($this->config->get('config_telephone'));
    $this->setTelephoneWork2($this->config->get('config_telephone'));
    $this->setTelephoneHome1($this->config->get('config_telephone'));
    $this->setTelephoneHome2($this->config->get('config_telephone'));
    $this->setCellphone($this->config->get('config_telephone'));
    $this->setCarphone($this->config->get('config_telephone'));
    $this->setPager($this->config->get('config_telephone'));
    $this->setAdditionalTelephone($this->config->get('config_telephone'));
    $this->setFaxWork($this->config->get('config_fax'));
    $this->setFaxHome($this->config->get('config_fax'));
    $this->setISDN($this->config->get('config_telephone'));
    $this->setPreferredTelephone($this->config->get('config_telephone'));
    $this->setTelex($this->config->get('config_telephone'));
    $this->setWorkStreet($this->config->get('config_address'));
    $this->setHomeStreet($this->config->get('config_address'));
    $this->setPostalStreet($this->config->get('config_address'));
    $this->setURLWork($this->config->get('config_url'));
    $this->setEMail($this->config->get('config_email'));
    $this->writeCardFile();
  }
  ...
  */
  function deleteOldFiles($time = 300)
    ...
    while ($file = readdir($handle))
      {
      if (!preg_match("/^\.{1,2}$/",$file) && !is_dir($this->download_dir . '/' . $file) && preg_match("/\.vcf/",$file) && ((time() - filemtime($this->download_dir . '/' . $file)) > $time))
        {
